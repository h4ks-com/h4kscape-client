...
class Game extends Client {
    ...
    private loadTitleImages = (): void => {
        ...
        this.showProgress(10, 'Connecting to fileserver').then((): void => {
            ...
            // Auto-login from stored OAuth session (redirect fallback)
            ...
            if (storedSession) {
                try {
                    ...
                    if (session.username && session.sessionToken) {
                        this.titleScreenState = 4;
                        this.loginMessage0 = '';
                        this.loginMessage1 = 'Connecting to server...';
                        this.login(session.username, session.sessionToken, false);
                    }
                } catch (_) {
                    localStorage.removeItem('oauth_session');
                }
    ...
    private updateTitleScreen = async (): Promise<void> => {
        if (this.titleScreenState === 0) {
            // OAuth: single Login button centered
            const x: number = (this.width / 2) | 0;
            const y: number = ((this.height / 2) | 0) + 20;
            ...
            if (this.mouseClickButton === 1 && this.mouseClickX >= x - 75 && this.mouseClickX <= x + 75 && this.mouseClickY >= y - 20 && this.mouseClickY <= y + 20) {
                ...
                // Check for stored OAuth session (fallback from callback redirect)
                const storedSession = localStorage.getItem('oauth_session');
                if (storedSession) {
                    try {
                        const session = JSON.parse(storedSession);
                        ...
                        this.loginMessage1 = 'Connecting to server...';
                        this.titleScreenState = 4;
                        await this.login(session.username, session.sessionToken, false);
                        return;
                    } catch (_) {
                        localStorage.removeItem('oauth_session');
                    }
                ...
                const oauthConfig = (window as any).__oauthConfig;
                if (oauthConfig && oauthConfig.enabled) {
                    this.titleScreenState = 4;
                    this.loginMessage0 = '';
                    this.loginMessage1 = 'Waiting for login...';
                    await this.openOAuthPopup();
                } else {
                    // Fallback: go to username/password form
                    this.loginMessage0 = '';
        ...
        } else if (this.titleScreenState === 4) {
            // OAuth waiting state — Cancel button
            const x: number = (this.width / 2) | 0;
            const y: number = ((this.height / 2) | 0) + 50;
    ...
    private drawTitleScreen = async (): Promise<void> => {
        ...
        if (this.titleScreenState === 0) {
            ...
            if (this.loginMessage0.length > 0) {
                this.fontBold12?.drawStringTaggableCenter(x, y - 10, this.loginMessage0, Colors.YELLOW, true);
                this.fontBold12?.drawStringTaggableCenter(x, y + 5, this.loginMessage1, Colors.YELLOW, true);
            } else {
                this.fontBold12?.drawStringTaggableCenter(x, y, this.loginMessage1, Colors.YELLOW, true);
            }
    ...
    // ---- OAuth PKCE popup flow ----

    private oauthPopup: Window | null = null;
    private oauthMessageHandler: ((event: MessageEvent) => void) | null = null;

    private openOAuthPopup = async (): Promise<void> => {
        const config = (window as any).__oauthConfig;
        if (!config || !config.enabled) {
            this.loginMessage0 = 'OAuth is not configured.';
            this.loginMessage1 = 'Please contact the server administrator.';
            this.titleScreenState = 0;
            return;
        }
        ...
        const codeChallenge = this.base64UrlEncode(new Uint8Array(challengeBuffer));
        ...
        const authUrl = config.logtoEndpoint + '/oidc/auth?' + params.toString();
        ...
        this.oauthMessageHandler = (event: MessageEvent): void => {
            if (event.data?.type !== 'oauth_callback') return;
            const { username, sessionToken } = event.data;
            if (username && sessionToken) {
                this.loginMessage0 = '';
                this.loginMessage1 = 'Connecting to server...';
                this.login(username, sessionToken, false);
            } else {
                this.loginMessage0 = 'Login failed.';
        ...
        if (!this.oauthPopup) {
            this.loginMessage0 = 'Could not open login window.';
            this.loginMessage1 = 'Please allow popups for this site.';
            this.titleScreenState = 0;
